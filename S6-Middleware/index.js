...
// Funció 1:
/* El mètode express.json() del paquet express ens permet parsejar els objectes a JSON */
...
// El que farem és extreure la funció Middleware anterior i posar-la en un mòdul extern per 
// de cridar-la des d'aquí. El mòdul serà myDate.js => const myDate = require('./myDate);
...
// Funció 4:
/* Ara creem un middleware que només es cridarà si l'endpoint que hem sol.licitat és
   '/api/cars/list'. Si es crida qualsevol altre endpoint, no passarà per aquí. */
...
app.use('/api/login', function(res, req, next){
    console.log('Logging');
    next();
})
...
app.get('/api/cars/id/:myId', (req, res) => {
...
app.get('/api/cars/:myCompany', (req, res) => {
...
/* POST amb validacions. Canviem l'endpoint a /api/cars2/ per distingir-lo. En el cas de POST
...
/* POST amb express-validator. Canviem l'endpoint a /api/cars3/ per distingir-lo. En el cas de POST
   els paràmetres els estem passant a través del BODY, no de l'url. Per això utilitzem 
   req.body.myCompany, req.body.myModel, req.body.myYear */
...
/* PUT amb express-validator. Canviem l'endpoint a /api/cars/::myId. En el cas de PUT
   els paràmetres els estem passant a través de l'URL, no del BODY. Per això utilitzem
   req.params.myId */

app.put('/api/cars/:myId', [
    // myCompany must be at least 4 chars long
    body('myCompany').isLength({ min: 4 }),
    // myModel must be at least 1 char long
    body('myModel').isLength({ min: 1 }),
...
app.delete('/api/cars/:myId', (req, res) => {
